...
| Spin Master | Clear N bricks with penetrating spin | ✅ | Gentle — cap ~5 |
| Opening Salvo | Break N bricks in first 15 seconds | ✅ | Yes — raise N |
| Score Target | Score X points this wave | ✅ | Yes — scale with wave |
...
- After Survival wave generation, call objective assignment.
- Emit new signals: `objective_assigned`, `objective_progress`, `objective_completed`, `objective_failed`.
- On wave completion, award bonus if objective was met.
...
- Add objective display element (label in top bar area).
- Connect to objective signals for progress/completion/failure updates.
...
- All values need playtesting — these are starting points.
...
- Track: final score + rows survived (number of pushes withstood).
- Both values shown on game over screen and leaderboard.
...
| Key | Type | Purpose |
|-----|------|---------|
| `blitz_top_runs` | `Array[{score, rows_survived, difficulty, date}]` | Top 10 Blitz runs per profile |
...
| Function | Purpose |
|----------|---------|
| `save_blitz_run(score, rows_survived)` | Persist a Blitz run result |
...
| `blitz_best_rows` | Highest rows survived in a single Blitz run |
...
### Current Problem
The High Scores screen already has a double-tab issue on the Sets board. Adding a Blitz tab makes this worse.

### Minimum Viable Change
- Add a "Blitz" tab alongside the existing "Survival" tab in the High Scores screen.
- Blitz entries show: rank, profile name, score, rows survived, difficulty.
...
4. Complete an objective → confirm flash/checkmark and bonus awarded (visible in score).
5. Fail an objective → confirm quiet fade/strikethrough, no penalty.
6. Confirm layout-dependent objectives (Bomb Chain) only appear on waves with enough bombs.
7. Confirm objectives scale in later waves (higher combo targets, tighter timers).
8. Test on Easy/Normal/Hard — confirm difficulty multipliers apply to objective score rewards.
...
3. Wait for push timer → confirm all bricks shift one column rightward and new row appears at left.
4. Confirm push is instant (no animation).
5. Let bricks reach the paddle zone → confirm game over triggers.
6. On game over → confirm score and rows survived are displayed.
7. Confirm no unbreakable bricks appear anywhere in Blitz.
8. Confirm ball bounces off bricks that snap into its position during a push.
9. Pause during gameplay → confirm push timer pauses. Unpause → timer resumes.
10. Test on Easy/Normal/Hard — confirm speed and score multipliers apply.
11. Play a long run → confirm push timer tightens over time (starts ~15-20s, gets faster).

#### Save Data
1. Launch game with pre-existing save (no `blitz_top_runs` key) → confirm no crash, key auto-initialized.
2. Complete a Blitz run → confirm run appears in `blitz_top_runs` save data.
3. Confirm Survival runs are still saved correctly (no regression).
4. Check stats screen for new Blitz/objective stats.

#### Leaderboard
1. Open High Scores → confirm Blitz tab is present.
2. Complete Blitz runs → confirm they appear on the Blitz leaderboard with score and rows survived.
